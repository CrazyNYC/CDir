...
def main():
    ...
    while any(flag.lower() in sys.argv for flag in (['/extsum'])):
        no_ext_sum = False
        sys.argv.remove('/extsum')
    ...
    while any(flag.lower() in sys.argv for flag in (['/valsum'])):
        no_val_sum = False
        sys.argv.remove('/valsum')
    ...
    # if /colorfileonly is specified on the command-line
    ...
    if not show_help:
        ...
        for raw in proc.stdout:
            ...
            if any(ext in line for ext in ["<DIR>", "<JUNCTION>", "<SYMLINK>", "<SYMLINKD>", "<REPARSE>"]):
                ...
                # line += '\t' if len(f"{p_filename}{p_ext}") < 8 else ""
                ...
                metadata = get_metadata(p_ext, p_size, force_update_cache) if ok_2_get_metadata else ""
...
#TODO treat SYMLINK as files (as per DIR in # File(s) totals)
        ...
        if ext_cache_ctr and len(ext_cache_ctr) > 0:
            ...
            print(f" \n{co.BLDYELLOWFG}All extensions encountered [{co.BLDGREENFG}{len(ext_cache_ctr)} "
                ...
                if not no_ext_sum or not no_val_sum or not no_ext_info_sum else None
            ...
            if ext_cache_ctr and len(ext_cache_ctr) > 1:
                ...
                for ext, val in ext_cache_ctr.items():
                    ...
                    for key2, val2 in sorted(groups_size.items(), key=lambda item: item[1], reverse=True):
                        ...
                        exts = wrap_at_nearest_space(exts, max_width=80, comma_color=comma_color)
                        parts.append(
                            f"\n\t {num_color}{val2[0]:>17,} {arrow_color}[{num_color}{get_short_size(val2[0])}{arrow_color}] {arrow_color}-> {ext_color}{exts} {arrow_color}"
                            f"{max(8 - len(strip_ascii(exts).strip(', ')), 0) * '-'}-> ({arrow_color}{ext_cache_ctr[key2]:,}"
                            f"{arrow_color}){co.ENDC}")

                    joined += (f" \n \n{co.BLDREDFG}Ext. by size:\n \n{co.WHITEFG} TB  GB  MB  kb bytes\n{co.DIMWHITEFG}--- --- --- --- ---{co.ENDC}")
                    joined += "".join(parts) if not no_val_sum else ""
                ...
                print(f"\t{' '*20}{co.BLDWHITEFG}{'-'*7}\n\t{' '*18}{ttl_value:>9,} {co.DIMREDFG}->{co.BLDWHITEFG} Total file(s){co.ENDC}") \
                    if not no_ext_info_sum and no_ext_sum else None

                # print ttl size
                print(f"{' '*2}{co.BLDWHITEFG}{'-' * 17}\n{num_color}  {ttl_size:>17,} [{num_color}{get_short_size(ttl_size)}{co.BLDWHITEFG}]{co.DIMREDFG} ->{co.BLDWHITEFG} Total size{co.ENDC}") \
                    if (not no_ext_info_sum and no_val_sum) else None
        ...
        print(f"{co.BLDWHITEFG}      /noextsum        {co.DIMWHITEFG}Suppresses file count summary{co.ENDC}")
        print(f"{co.BLDWHITEFG}      /extsum          {co.DIMWHITEFG}Forces file count summary{co.ENDC}")
        print(f"{co.BLDWHITEFG}      /novalsum        {co.DIMWHITEFG}Suppresses file size summary{co.ENDC}")
        print(f"{co.BLDWHITEFG}      /valsum          {co.DIMWHITEFG}Forces file size summary{co.ENDC}")
...
def get_metadata(p_ext: str, p_size: str, force_update_cache: bool = False) \
        -> str | None:
...
def color_2use(p_line: str = "",
    ...
    # PYTHON and app related files (perhaps later we add all source-code-related files)
            ...
            (".py", ".pyc", ".c", ".cpp", ".h", ".lib", ".msc", ".pyi", ".pyd", ".pyz", ".pyzw", ".toc",
             ...
             ".copying", ".head", "py.typed", "readme.md", ".gitignore", ".csproj", ".xhtml",
